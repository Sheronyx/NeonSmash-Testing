...
// Zentraler Dienst (Singleton, EIN Objekt in der Szene): lässt ein frisch instanziiertes Element
// von außerhalb des rechten Bildschirmrands einfliegen — startet winzig an einem zufälligen Punkt
// knapp außerhalb (Höhe variiert, aber nie ganz unten), fliegt über eine zufällige Bogenkurve zur
// Zielposition (quadratische Bezier, wie die Activation-Orb-Zeremonien) UND wächst dabei in EINER
// durchgehenden Bewegung auf Originalgröße (Skalierung per Ease-Out-Back — überschwingt von selbst
// leicht und landet exakt bei Zielgröße, keine separate Pop-In-Phase mehr nötig). Danach
// onArrived-Callback — der Aufrufer startet dort Timer/Fuse/Pulse. Collider des Ziel-Objekts bleibt
// während der gesamten Bewegung deaktiviert, damit ein noch fliegendes Element nicht antippbar ist.
//
...
public class PointFlyIn : MonoBehaviour
{
    ...
    [Tooltip("Erlaubter Höhenbereich am rechten Bildschirmrand (Viewport 0 = unten, 1 = oben). Standard " +
             "schließt \"ganz unten\" aus.")]
    [SerializeField] private float minSpawnHeight = 0.25f;
    [SerializeField] private float maxSpawnHeight = 0.95f;
    ...
    // target = das bereits instanziierte Element, das einfliegen soll.
    ...
    public void Play(GameObject target, Vector3 targetPosition, Vector3 targetScale, Action onArrived)
    {
        ...
        StartCoroutine(Co_Fly(target, col, startPos, ctrl, startScale, targetPosition, targetScale, onArrived));
    }

    private IEnumerator Co_Fly(GameObject target, Collider2D col, Vector3 startPos, Vector3 ctrl,
                                Vector3 startScale, Vector3 targetPos, Vector3 targetScale, Action onArrived)
    {
        ...
        while (t < duration)
        {
            // Ziel kann zwischenzeitlich zerstört werden (z.B. Spielende) — dann Coroutine sauber beenden.
            ...
            float u    = 1f - posK;
            tr.position = u * u * startPos + 2f * u * posK * ctrl + posK * posK * targetPos;
    ...
    // Zufälliger Punkt knapp außerhalb des rechten Bildschirmrands — Höhe variiert innerhalb
    // [minSpawnHeight, maxSpawnHeight], aber nie ganz unten.
    private Vector3 OffScreenPos()
    {
        ...
        float u    = UnityEngine.Random.Range(minSpawnHeight, maxSpawnHeight);
        Vector2 vp = new Vector2(1f + offScreenMargin, u);
