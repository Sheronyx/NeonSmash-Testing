...
// Aktivierungs-Zeremonie für Vortex Mode — 1:1 identisch zu GravityModeActivationPoint (fliegt zum
// Arcane Portal, wie alle anderen Special-Mode-Orbs). Die eigentlichen Vortex-Elemente im Gameplay
// saugen separat zum VortexCenter — nur diese Orb-Einleitung bleibt konsistent mit Gravity/Fountain.
public class VortexModeActivationPoint : MonoBehaviour
{
    ...
    [Header("Portal")]
    [SerializeField] private float delayBeforeVortexMode = 1.6f;
    ...
    private void FinishCombo()
    {
        ...
        if (spawner != null)
        {
            if (SpecialModeManager.Instance != null && !SpecialModeManager.Instance.IsModeActive)
                SpecialModeManager.Instance.StartMode(SpecialMode.Vortex);
